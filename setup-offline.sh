...
# Values are quoted: the manual role scripts `source` this file, and a Windows profile
# path with a space would otherwise run its tail as a command (upstream issue #781).
...
# --- [4e] python3-обёртка (git bash Windows: есть python, нет python3) ---
# Скрипты и хуки зовут python3 (rule-engine, check-index-health, fp-stats).
# В git bash на Windows есть только python. alias НЕ работает в неинтерактивных
# shell/хуках (резолв через PATH), поэтому кладём обёртку python3→python в ~/bin
# и гарантируем ~/bin в PATH. Идемпотентно.
echo "[4e] python3-обёртка (git bash)..."
if command -v python3 >/dev/null 2>&1; then
  echo "  ○ python3 уже доступен — обёртка не нужна"
elif ! command -v python >/dev/null 2>&1; then
  echo "  ⚠ ни python3, ни python не найдены — Python-шаги (rule-engine, check-index-health, fp-stats) работать не будут"
elif $DRY_RUN; then
  echo "  [dry-run] создал бы $HOME/bin/python3 → python + добавил ~/bin в PATH (~/.bashrc)"
else
  mkdir -p "$HOME/bin"
  cat > "$HOME/bin/python3" <<'PYW'
#!/bin/sh
exec python "$@"
PYW
  chmod +x "$HOME/bin/python3"
  if ! grep -qF 'IWE: ensure ~/bin on PATH (python3 wrapper)' "$HOME/.bashrc" 2>/dev/null; then
    cat >> "$HOME/.bashrc" <<'RC_EOF'

# IWE: ensure ~/bin on PATH (python3 wrapper)
case ":$PATH:" in *":$HOME/bin:"*) ;; *) export PATH="$HOME/bin:$PATH" ;; esac
RC_EOF
  fi
  export PATH="$HOME/bin:$PATH"   # для текущего запуска
  echo "  ✓ $HOME/bin/python3 → python (перезапусти git bash или: source ~/.bashrc)"
fi

# --- [4f] jq для хуков защиты секретов ---
# Without jq the secret-* hooks would block every tool call; the shim turns that into
# a visible "protection off" warning instead (MANUAL-JOBS.md). Offline we cannot fetch
# jq, so only report it here — at install time, not first in the middle of a session.
echo "[4f] jq (хуки защиты секретов)..."
if command -v jq >/dev/null 2>&1; then
  echo "  ○ jq доступен"
else
  echo "  ⚠ jq не найден — защита от утечки секретов будет выключена."
  echo "    Положите jq.exe в ~/bin (или другой каталог из PATH), см. MANUAL-JOBS.md."
fi
