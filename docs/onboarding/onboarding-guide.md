...
> ⚠️ **Документ предшествует WP-287 (май 2026).** Актуальная публичная документация для новых пользователей живёт в `DS-ecosystem-development/.../C2.1.Meaning/2.1.2. Onboarding/`:
> - `iwe-what-is.md` — что такое IWE и зачем (канонический ответ для холодного канала)
> - `iwe-quickstart.md` — быстрый старт за 15 минут
>
> Этот файл остаётся в репо как концептуальное введение и используется для пользователей, которые уже клонировали шаблон.
...
| Планы не работают | **Культура работы** (ритуалы ОРЗ + Qwen Code) | Утром — план дня (автоматически). Вечером — итоги. Каждую неделю — ревью. ИИ не даёт забыть незавершённое |
...
1. Установить Qwen Code (бесплатно)
2. Создать аккаунт на GitHub (бесплатно)
3. Запустить одну команду, которая настроит всё остальное

> **ИИ поможет.** Если ты уже поставил Qwen Code — просто скажи ему: «Помоги мне установить IWE». Он проведёт тебя через каждый шаг.
>
> Подробная инструкция: [SETUP-GUIDE.md](../SETUP-GUIDE.md)

**Что нужно (минимум):**
- Компьютер (Mac, Linux или Windows с WSL)
- Подписка Claude Pro (~$20/мес) — для Qwen Code
- GitHub аккаунт (бесплатно)

### Шаг 3. Первая стратегическая сессия (~30 минут)

После установки ты запускаешь Qwen Code и проводишь первую сессию:
...
- **Терминал** — окно, в которое ты вводишь команды текстом (Qwen Code подскажет, что вводить)
- **CLI** — просто способ общения с компьютером через текст вместо кнопок

После установки ты будешь взаимодействовать в основном с Qwen Code — на обычном русском языке.
...
|------|----------------|---------------|
| **Этап 1 — Старт** | Qwen Code + экзокортекс | Персональный ИИ-ассистент, который тебя помнит |
...
**Экзокортекс** — твоя вторая память. Набор файлов, которые ИИ-ассистент (Qwen Code) читает и обновляет. Когда ты начинаешь работу — он знает, на чём ты остановился вчера. Когда заканчиваешь — записывает выводы. Ты больше не теряешь контекст между сессиями.
...
В центре IWE — **ты**. Ты принимаешь решения, мыслишь, направляешь. Qwen Code — твой напарник: он усиливает, структурирует, берёт на себя рутину. Но решения — всегда за тобой.

### Инструменты (средства доставки)

Инструменты — это не IWE. Это **средства доставки** четырёх компонентов:

**Qwen Code** — ИИ-ассистент, который работает прямо в терминале. Он не просто отвечает на вопросы — он читает твои файлы, помогает планировать, напоминает о незавершённых делах. Это твой напарник, а не поисковик.

**GitHub** — облачное хранилище с полной историей изменений. Ты всегда можешь вернуться к любой версии любого файла.

**VS Code** — бесплатный редактор. На старте не нужен — Qwen Code работает в терминале. VS Code понадобится позже, когда захочешь самостоятельно просматривать файлы.
...
> **Инструменты** (Qwen Code, VS Code, GitHub, бот) — это средства доставки. Как железо, на котором работает ОС.
