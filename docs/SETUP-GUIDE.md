...
2. **Все шаги Этапа 0 и Этапа 1** (Node.js, GitHub CLI, Qwen Code CLI, `setup.sh`) выполняй **из Git Bash**, не из PowerShell/cmd — команды с `curl`, `xcode-select` и т.п. в PowerShell не заработают.
   ...
   - Qwen Code CLI — та же команда `npm install -g @anthropic-ai/claude-code`, что и на macOS/Linux (Git Bash уже умеет `npm`, если Node.js на PATH).
3. **Автоматические хуки Qwen Code** (пре/пост-commit и т.п.) вызывают `.sh`-файлы через системный shell — на Windows это сработает, только если `bash` (из Git for Windows) есть в системном `PATH`. Установщик Git for Windows обычно добавляет его сам; если хуки не срабатывают — проверь `where bash` в cmd.
...
> Ниже — путь для Qwen Code (через `claude.ai`). Для других агентов (Hermes и т.п.) см. `AGENT-VENDOR-SETUP.md`, Шаг 6.
...
`.qwen/skills/personal-guide-start/SKILL.md` и перезапусти Qwen Code. Если
