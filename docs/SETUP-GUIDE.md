...
| **T4: Созидание (IWE)** | + Qwen Code, Стратег, Git, свои базы знаний | **Это руководство** |
...
- **Qwen Code** — ИИ-помощник, который знает твои цели, задачи и методологию. Помнит контекст между сессиями
...
|------|-----|-------|---------------------|
| **0** | Подготовка (Git, Node, Qwen Code) | 15-20 мин | **обязательно** |
...
Если у тебя уже установлены Git, Node.js, GitHub CLI и Qwen Code CLI — переходи к [Этапу 1](#этап-1-установка-iwe-5-мин).
...
Node.js — среда выполнения JavaScript. Нужна для установки Qwen Code CLI. npm — менеджер пакетов Node.js (устанавливается вместе с Node.js).
...
Qwen Code — это ИИ-агент, работающий в терминале (или в VS Code). Он читает файлы, выполняет команды, помогает планировать и писать код.
...
При первом запуске Qwen Code попросит войти в аккаунт Anthropic — следуй инструкциям в терминале.

### 0.5b Оптимизация стоимости: выбор модели

Qwen Code позволяет выбирать модель для каждой задачи. Правильный выбор экономит лимит подписки:
...
Переключить модель в Qwen Code: `/model` → выбрать. Для автоматических задач (Стратег, Экстрактор) рекомендуется Haiku — экономит ~80% лимита по сравнению с Opus.
...
VS Code — редактор кода с графическим интерфейсом. В нём удобно работать с Qwen Code: видишь все свои репо, файлы, терминал и ИИ-ассистента в одном окне и можешь в одной сессии менять репо разных проектов. **Без VS Code** придётся работать только через терминал — это возможно, но менее наглядно.

- Скачай и установи: [code.visualstudio.com](https://code.visualstudio.com/)
- Открой VS Code → нажми `Cmd+Shift+X` (macOS) или `Ctrl+Shift+X` (Windows/Linux) → найди «Qwen Code» → нажми Install
...
Создай на своём компьютере **одну папку** для всех репозиториев — текущих и будущих. В неё будут клонироваться все репозитории: `FMT-exocortex-template/`, `DS-strategy/`, `PACK-{область}/`, `DS-{проекты}/` и др. `QWEN.md` тоже будет лежать в корне этой папки. По умолчанию это `~/IWE`:
...
> **Важно:** Название может быть любым, но все репо должны быть в одном месте — Qwen Code ориентируется на эту структуру. Рекомендуем `~/IWE`.
...
2. Установит `QWEN.md` — правила для Qwen Code
3. Установит `memory/` — оперативную память для Qwen Code
4. Настроит разрешения (`.qwen/settings.local.json`) и выведет инструкцию по подключению MCP
...
MCP (Model Context Protocol) -- это доступ Qwen Code к базе знаний платформы и твоим личным репозиториям. Через него Claude видит документы, руководства, цифровой двойник и твои собственные Pack-репо — предметные базы знаний, которые ты создаёшь со временем.
...
3. Перезапусти Qwen Code

**Как работает:** Qwen Code подключается к Aisystant MCP через claude.ai connectors. Сервер агрегирует все бэкенды (knowledge, digital-twin) и предоставляет инструменты (`knowledge_search`, `knowledge_get_document`, `knowledge_feedback`, `dt_read_digital_twin` и др.).

#### Проверка

Открой Qwen Code в папке экзокортекса и набери `/mcp` -- серверы должны быть в статусе Connected. Затем попроси:
...
| Открыл URL в браузере -- "Not found" | Нормально. MCP работает по POST (JSON-RPC), не по GET. Проверяй через `/mcp` в Qwen Code |
...
**`QWEN.md` не найден:**
```bash
cp ~/IWE/FMT-exocortex-template/QWEN.md ~/IWE/QWEN.md
```
...
mkdir -p ~/.qwen/projects/-Users-ivan-IWE/memory
cp ~/IWE/FMT-exocortex-template/memory/*.md ~/.qwen/projects/-Users-ivan-IWE/memory/
```
...
# 4. Перезапустить Qwen Code → память на месте
```

Скрипт: копирует `exocortex/*.md|*.yaml` → auto-memory (`~/.qwen/projects/<slug>-IWE/memory/`), `exocortex/QWEN.md` → `~/IWE/QWEN.md`, создаёт симлинк `~/IWE/memory → auto-memory`. Непустую `memory/` без `--force` не трогает (защита от случайной перезаписи рабочей инсталляции).
...
3. Открой панель Qwen Code: `Cmd+Shift+P` (macOS) или `Ctrl+Shift+P` (Windows) → набери «Qwen Code: Open» → Enter
...
Claude прочитает QWEN.md и memory/ и проведёт тебя через:
...
В VS Code или терминале запусти Qwen Code и скажи:
...
Подключение Google Calendar позволяет видеть события дня прямо в утреннем плане, создавать события из Qwen Code и готовиться к встречам.

### Что получишь

- **Day Open** покажет таблицу событий дня + свободные слоты для работы
- **Создание событий** — «поставь созвон на среду 11:00» прямо из Qwen Code
...
4. Перезапусти Qwen Code → проверь: **«покажи мои события на сегодня»**
...
Если ты только начал работу с IWE и используешь только Qwen Code в интерактивном режиме — **тебе это НЕ нужно**. Все отчёты планировщика будут храниться в `DS-strategy/current/` и `DS-strategy/archive/` — этого достаточно.
...
Каждая сессия в Qwen Code проходит три стадии:
...
QWEN.md (§1-7), memory/ (протоколы, справочники), промпты и скрипты ролей, hooks, скиллы, setup-скрипты. Если скрипты ролей изменились — автоматически переустановятся launchd-агенты.

**Что НЕ трогается (user-space):**
- QWEN.md — 3-way merge: ваши правки в любой секции сохраняются при обновлении
...
- .qwen/settings.local.json — персональные permissions
- personal/ — ваши файлы
...
|-----------|-------------|------------------------|
| QWEN.md, memory/ | Локальные файлы | Нет (только в контекст Claude при работе) |
...
|-----------|------|-------------|
| **Qwen Code** | Anthropic API (США) | Промпты, содержимое файлов из контекста. [Privacy Policy](https://www.anthropic.com/privacy) |
...
Да, Qwen Code требует подписку Anthropic. Рекомендуется начать с **Claude Pro** ($20/мес). При необходимости — **Claude Max** (~$100/мес).
...
Да. Стратег — это автоматизация (утренние планы, ревью). Без него Qwen Code + QWEN.md + memory/ работают полностью. Планируешь вручную.
...
**Как проверить MCP?**
`/mcp` в Qwen Code -- серверы должны быть Connected. Попроси: «Найди документы про принципы». Не работает? Запусти `bash FMT-exocortex-template/setup.sh --validate` -- покажет что именно сломано. Подробности -- см. шаг 1.3b.
