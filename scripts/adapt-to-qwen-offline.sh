...
# Остальные CLAUDE.md (seed/, templates/, наборы-заготовки внутри скиллов) — одним
# правилом вместо списка путей. Список ломался на каждом новом скилле, который везёт
# собственный шаблон инструкций: шаг 2 переписывал ВСЕ ссылки на него в QWEN.md, а сам
# файл оставался CLAUDE.md — манифест и скилл указывали на несуществующий файл.
# Так в цикле 10 приехал .qwen/skills/repo-new/assets/repository-skeleton/ (нашло ревью).
# Идёт после переименования каталога, чтобы пути уже были финальными.
while IFS= read -r -d '' f; do
  rel="${f#./}"
  do_mv "$rel" "${rel%CLAUDE.md}QWEN.md"
done < <(find . \( -path './.git' -o -path './.git/*' \) -prune -o \
           -type f -name 'CLAUDE.md' -print0)
  ...
  while IFS= read -r -d '' f; do
    case "$f" in
      *CHANGELOG.md) continue ;;
      # Переводческий глоссарий — контент о платформе, а не её конфигурация:
      # «Слот exocortex/CLAUDE.md» там термин статьи, а не путь, по которому кто-то ходит.
    ...
    ' "$f"
  # Отбор шире, чем «файлы с известным расширением». Прежний набор молча пропускал
  # три класса, и каждый из них ломался функционально (нашло ревью цикла 10):
  #   • без расширения — .githooks/* : фильтры областей внутри хуков остались на
  #     '^\.claude/', поэтому коммит, трогающий только .qwen/, обходил проверки;
  #   • .list / .txt — scripts/hot-files.list (аудитор бюджета искал .claude/rules/*)
  #     и эталон scripts/tests/fixtures/python-resolver-baseline.txt, который
  #     сверяется со сканом уже адаптированного дерева и потому падал целиком;
  #   • .example и точечные файлы (.stignore, .orphan-allowlist) — инструкции и
  #     списки игнорирования, уводившие пользователя в несуществующий .claude/.
  done < <(find . \( -path './.git' -o -path './.git/*' \) -prune -o \
             -type f \( -name '*.sh' -o -name '*.py' -o -name '*.yaml' -o -name '*.yml' \
                     -o -name '*.json' -o -name '*.md' -o -name '*.list' -o -name '*.txt' \
                     -o -name '*.example' -o -name '.stignore' -o -name '.orphan-allowlist' \
                     -o -path './.githooks/*' \) -print0)
...
update-manifest.local.json
.qwen/hooks/qwen-tool-name-shim.sh
"
