...
if [[ "$MODE" != "settings-json" ]]; then
    ...
    for f in "${TARGETS[@]}"; do
        ...
        # файлы .qwen/hooks/tests/*.sh — сами настоящие исполняемые тест-
        ...
        case "$f" in
            scripts/tests/*|*/scripts/tests/*) skip_hardcode_check=1 ;;  # issues #446/#450: фикстуры конвенции scripts/tests/ — не сканировать. Уже, чем */tests/* — другие tests/-каталоги репо (.qwen/skills/*/tests/, guide-kit/tests/) продолжают проверяться как обычно.
            ...
            .qwen/hooks/tests/*|*/.qwen/hooks/tests/*) skip_hardcode_check=1 ;;  # issue #759: фикстуры хук-тестов — тот же класс, что scripts/tests/.
        esac
