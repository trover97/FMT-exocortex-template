...
CHECK_ONLY=false
mkdir -p "$SCRIPT_DIR/.qwen" "$WORKSPACE_DIR/.qwen"

report_settings_merge_preview() {
    printf '%s|%s\n' "$1" "$2" > "$TMP/preview-call"
}

printf '{"hooks":{"Stop":[]}}\n' > "$SCRIPT_DIR/.qwen/settings.json"
printf '{"hooks":{"Stop":[],"SessionStart":[]}}\n' > "$WORKSPACE_DIR/.qwen/settings.json"
...
if [ "$(cat "$TMP/preview-call")" != "$SCRIPT_DIR/.qwen/settings.json|$WORKSPACE_DIR/.qwen/settings.json" ]; then
    echo 'settings drift did not invoke preview for the template/workspace pair' >&2
    exit 1
fi

rm "$TMP/preview-call"
cp "$SCRIPT_DIR/.qwen/settings.json" "$WORKSPACE_DIR/.qwen/settings.json"
