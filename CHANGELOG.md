...
- Прикладная дорожка guide-kit (генератор занятий получил v0.1.3): планировщик, декомпозер, интеграция каталога F6/F10, адаптер здоровья, формализация исследования, роутер сценариев, поиск источников (РП-483 Ф6-Ф10, РП-495 Ф5b).
- Гейт готовности резидентуры (`residency-gate-skill-adapter.sh`): проверка согласия на использование данных теперь срабатывает автоматически на каждый вызов скилла через хук, а не только когда автор скилла сам подключил проверку (`#323`).
- L2-триггер Экономичности в АрхГейте: новый автоматический процесс или рост частоты/объёма обращений к LLM теперь явно проверяется чек-листом (§4.8-4.15, WP-507 Ф0.1).

### Fixed

- Планировщик занятий: пользователь без рассчитанного профиля мастерства получал придуманную моделью тему вместо реального занятия из каталога - добавлен запасной путь через соседние области (РП-406).
- Разложение прикладных источников на занятие переставало работать при пустом, но объявленном списке разделов - терялся журнал решений (РП-406).
- Хуже проверенные места обновления шаблона закрыты (`#384`, `#386`-`#391`): пути раскрытия окружения, память ролей и различений, генератор списка горячих файлов.
- Устаревшие ссылки на терминологию FPF (первопринципный каркас) вычищены из памяти (`#393`); заодно досверен манифест под уже доставленные изменения гейта резидентуры и диспетчера.
- Проверка путей установки при обновлении шаблона (install-path guard) сужена до реального различия версий и защищена от ложных срабатываний на чужих строках диффа (`#385`).
- Гонка при параллельной работе нескольких агентов через очередь задач (`iwe-agent-dispatcher.py`) устранена: место в очереди занимается до начала работы, а не после.

### Changed

- Проверка на macOS в непрерывной интеграции переведена на ночной прогон и пул-реквесты вместо каждого обновления основной ветки - экономия около 80% минут запуска без потери покрытия (В1).
