...
> Kimi → `AGENTS.md`, Hermes → Aisystant MCP `get_instructions`; доставка ядра из авторского IWE в этот шаблон — внутренний авторский конвейер (не входит в шаблон, недоступен пользователю). Slim-ядро: триггеры + правила hot; детали → `memory/`, `.qwen/rules-lazy/`, `.qwen/skills/`.
...
> **SoT (WP-272 Ф1):** `PACK-agent-rules/rules/AR.NNN.md` (реестр `.qwen/rules-registry.yaml`) — авторский источник, не шипится в шаблон (генератор `.qwen/scripts/generate-rules-registry.py` требует `PACK-agent-rules`, которого у пользователя нет). На пользовательской установке — шипящаяся выжимка тех же 10 правил → `.qwen/rules-lazy/blocking-rules-full.md`. **Приоритет = нумерация:** структурное (1-5) перевешивает поведенческое (6-10).
...
6. **Hooks/Scripts Bypass Gate (S-33):** без явного разрешения не менять `.qwen/hooks|scripts/`, `.iwe-runtime/`, `FMT-exocortex-template/`, не обходить хуки; блок хука → bug-файл + пилоту + ждать. → `.qwen/rules-lazy/hooks-bypass-gate.md`.
7. **Автономность:** не спрашивать подтверждения — выполни → отчитайся. Исключения: необратимо-разрушительное; WP Gate Ритуал; choice-question. Полный текст → `.qwen/rules-lazy/blocking-rules-full.md` п.7.
...
9. **Финиш > отлог (S-46):** доп. задача → дефолт «делаю сейчас»; «сейчас или потом?» = анти-паттерн. Исключения и приоритет WP Gate → `.qwen/rules-lazy/blocking-rules-full.md` п.9.

### Протокол Работы (полный → `memory/protocol-work.md`)

**Capture-to-Pack** — на каждом рубеже: есть ли знание для записи? Анонс: *«Capture: [что] → [куда]»*. Маршрутизация: правило → QWEN.md, доменное → Pack, реализационное → DS docs/, урок → memory/; при новом артефакте Routing Gate (DP.KR.001 §5) первым.
...
> Полные формулировки → `.qwen/rules-lazy/blocking-rules-full.md`.
...
- Первое содержательное действие в репо → **Repo-Touch Gate:** прочитать `<repo>/QWEN.md`; блок «обязательно загружай» → загрузить ДО ответа.
...
- Новый инструмент/агент/система → **IntegrationGate (БЛОКИРУЮЩЕЕ):** только (1) обещание → (2) сценарии → (3) роль → (4) реализация → `.qwen/rules-lazy/integration-gate.md`.
- Замена legacy-компонента → **LegacyPortGate (БЛОКИРУЮЩЕЕ):** сначала 15-мин субагент «как это работает сейчас?» → `.qwen/rules-lazy/blocking-rules-full.md`.
...
> **БЛОКИРУЮЩЕЕ.** Архитектурное решение → `/archgate`: принципы DP.ARCH.001 §7 → профиль ЭМОГССБ (✅/⚠️/❌) → conjunctive screening; чеклист современности (SOTA.002/001/011 + CGUS/PUA) — внутри `.qwen/skills/archgate/SKILL.md`. Профиль без агрегатного балла — так и есть, это осознанный выбор (conjunctive screening, не средневзвешенное).

## 6. Форматирование → `.qwen/rules/formatting.md` · Различения → `.qwen/rules/distinctions.md`

## Контекстный бюджет IWE (WP-445)

Hot-каркас ≤20K токенов (M1), строгая цель ≤12K (M2). Изменил файл из `hot-files.list` (оба QWEN.md, rules/*.md) → перед коммитом `{{WORKSPACE_DIR}}/FMT-exocortex-template/scripts/verify-context-budget.sh`.

## 7. Обновление этого файла

> **3 слоя:** L1 (§1-§7) = платформа (`update.sh`). L2 (§8) = staging. L3 (§9) = авторское.
> Протоколы → `memory/protocol-*.md` · различения → `.qwen/rules/distinctions.md` · форматирование → `.qwen/rules/formatting.md` · стабильные знания → `memory/*.md` · свои правила → §8/§9.
...
> Правила на обкатке (STAGING.md) → работают → переносятся в шаблон (L1). Новое поведение в §9 → ОДНОВРЕМЕННО строка в STAGING.md (`status: testing`). Промоция на Week Close (`validated`→`promoted`, `rejected` остаётся в §9 — не удалять) → скилл `/author-mode` и `.qwen/rules-lazy/blocking-rules-full.md`.
...
- **Различения (авторские):** `.qwen/rules/distinctions.md` (секция «Авторские») + `memory/distinctions-warm.md` (в т.ч. «Бот = интерфейс, не место агентов»).
- **Именование:** governance-репо называется `DS-strategy` по умолчанию; рабочая директория `{{WORKSPACE_DIR}}/`.
- **Extensions Gate (БЛОКИРУЮЩЕЕ):** пользователи кастомизируют ТОЛЬКО `extensions/*.md` + `params.yaml` (правка `.qwen/skills/` или `memory/protocol-*.md` = ошибка); автор (`author_mode: true`) редактирует L1 напрямую — авторский IWE = SoT, доставка в FMT через `template-sync.sh`.
