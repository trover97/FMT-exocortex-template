...
> Slim-ядро: триггеры + правила. Детали → memory/protocol-*.md, .qwen/rules/, .qwen/skills/.
...
**Capture-to-Pack** — на каждом рубеже: есть ли знание для записи? Анонсировать: *«Capture: [что] → [куда]»*. Маршрутизация: правило (1-3 строки) → QWEN.md, доменное → Pack, реализационное → DS docs/, урок → memory/.
...
Политика: ≤11 файлов. **Горячие** (читаются каждую сессию: QWEN.md, MEMORY.md, distinctions.md, formatting.md): ≤100 строк. Протоколы (lazy, по триггеру): ≤150. **Lazy-reference** (по ссылке из MEMORY.md, не каждую сессию — feedback_*, templates-*, reference_*): без жёсткого лимита, > 300 строк → пересмотреть.
...
> **БЛОКИРУЮЩЕЕ.** Архитектурное решение → `/archgate` → принципы (DP.ARCH.001 §7) → профиль ЭМОГССБ (✅/⚠️/❌) → conjunctive screening (см. `.qwen/skills/archgate/SKILL.md`). Без агрегатного балла — `feedback_decision_gates.md`.
> Чеклист современности: (1) Context Engineering SOTA.002, (2) DDD Strategic SOTA.001, (3) Coupling Model SOTA.011.

## 6. Форматирование → `.qwen/rules/formatting.md`

## Различения → `.qwen/rules/distinctions.md`
...
- Различение (1-3 строки) → `.qwen/rules/distinctions.md`
- Форматирование → `.qwen/rules/formatting.md`
...
> Хранятся в `.qwen/rules/distinctions.md` в зоне AUTHOR-ONLY — не затираются при `update.sh`.
...
- **Flow (единый для всего L1):** авторский IWE (source-of-truth) → доставка в FMT (с отрезанием личного) → GitHub → `update.sh` → пользователи. Авторский IWE = SoT для ВСЕГО: QWEN.md, скриптов, хуков, скиллов.
- **QWEN.md:** авторский IWE → `bash $IWE_TEMPLATE/scripts/template-sync.sh` → автоматически в FMT (плейсхолдеры + отрезание §9). Режимы: без флагов = sync, `--dry-run` = diff, `--check` = drift (exit 1).
- **Промоция артефактов в шаблон** — единая команда по типу:
  ...
  - Хук: `bash $IWE_SCRIPTS/hook-promote.sh <личный-хук>.sh [--dry-run]` → FMT/.qwen/hooks/
  - Скилл: `bash $IWE_SCRIPTS/skill-promote.sh <папка-скилла>/ [--dry-run]` → FMT/.qwen/skills/
  - QWEN.md: `bash $IWE_SCRIPTS/template-sync.sh` (автозамена §9 + плейсхолдеры)
...
| QWEN.md | инструкции агента (только внутри, не для пользователя) |
