...
## Шаг 1. Контекстные приоритеты (обязательный)

**Спроси перед оценкой:** «Какие 1–2 характеристики критичны для этого решения?»

Если решение передано без явных приоритетов — задай вопрос. Не переходи к шагу 2 без ответа.

Примеры: «это auth-компонент → Безопасность критична», «это публичный API → Эволюционируемость + Безопасность», «это ML-пайплайн → Масштабируемость критична».

Пометь критические характеристики — ❌ в них = немедленный СТОП (правило блокировки #1).

> **Если передано несколько вариантов** — строй сравнительную таблицу профилей (см. шаг 2б).
...
Проверь правила блокировки по порядку:

| # | Условие | Действие |
|---|---------|----------|
| 1 | ❌ в характеристике, помеченной как **критическая** (шаг 1) | СТОП |
| 2 | ≥2 характеристики со статусом ❌ | СТОП |
| 3 | ≥4 характеристики ⚠️ при 0 ✅ | СТОП |
| — | Иначе | Проходит → шаг 4 |

**Если СТОП:**
> «Решение НЕ проходит АрхГейт. Блокирующие условия: [список]. Рекомендация: [что исправить].»

Перейди к шагу 5 (обратная связь) и завершай.
