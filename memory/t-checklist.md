...
| T5a | WP-карточка с `verification_class` ∈ {open-loop, problem-framing} и бюджетом ≥3h имеет секцию `## Этапы` | агент | в этой сессии создан/затронут такой РП | план по этапам не появляется в карточке — структурная дыра, найдено 18 из 20 проверенных карточек (structural-hole fix, см. `scripts/create-wp.sh --verification-class`) |

**Исключения:** сессии ≤15 мин БЕЗ изменений файлов → T1-T5 пропускаются (только устное summary).

**Верификация:** Haiku R23 проверяет формальное соответствие (есть ли commit, обновлён ли MEMORY.md, заполнен ли handoff). **T5a — предупреждение, не блок:** R23 не создаёт `/decompose` план сам (context isolation, другой субагент), только сообщает пилоту отсутствие секции; детерминированный гейт (`check-trace-satisfaction`) под это правило пока не реализован (issue #678), поэтому жёсткий блок здесь преждевременен.
