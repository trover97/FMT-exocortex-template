...
> число, только даёт единый список источник→значение→статус. Решение пилота
> (17.09): "≤11 файлов" убрано из `QWEN.md §4` целиком, не заменено новым
> числом или счётчиком — архитектура памяти давно перешла на HOT/WARM/COLD с
> бюджетом по токенам (строка ниже), раздельный лимит на число файлов не
> нужен поверх него.

| Порог | Значение | Источник | Статус |
|---|---|---|---|
| Число файлов памяти | — | — | **Снято** (issue #736, решение пилота 17.09) — текст "≤11 файлов" убран из `QWEN.md §4`: не имел кода-исполнителя, давно разошёлся с практикой, дублировал уже работающий бюджет по токенам ниже |
...
| `.qwen/rules/distinctions.md` (платформенный hot-слой) | ≤150 строк (канон выше) | `verify-context-budget.sh`, `.qwen/skills/week-close/SKILL.md` | **Исполняется** + drift-флаг Week Close — оба синхронизированы на 150 (issue #736; было 80 в week-close, расходилось с исполняемым порогом) |
...
| Collapsible-разметка файла-артефакта | >80 строк | `.qwen/rules/formatting.md` | **Совет** — форматирование (когда сворачивать секции в markdown), НЕ бюджет памяти distinctions.md; разные измеряемые, число совпадало с прежним (ошибочным) порогом distinctions.md только по случайности |
| MEMORY.md (индекс) | >200 строк | `.qwen/skills/week-close/SKILL.md:192` | **Совет** — агент проверяет глазами на Week Close; независимый критерий от размера в КБ ниже (строки ≠ байты/длина строки) — оставлено как есть, не сводится в одно число |
| Индекс-файл (`check-index-health.py`) | 30 КБ warn / 100 КБ fail, 300/500 симв/строка (warn/fail), 200/400 симв/ячейка (warn/fail) | `.qwen/scripts/check-index-health.py` | **Исполняется** — Day Close шаг 4в |
...
**Известные несведённые расхождения (не решается этой таблицей):** порог индекса на Week Close (200 строк, ручной) и в `check-index-health.py` (КБ+длина строки/ячейки, автоматический) — два разных измеряемых одного файла (строки vs байты/длина), намеренно оставлены раздельными, не одно число.
...
> пишет системная инструкция Qwen Code, запретить её нельзя — ЧИТАЕТСЯ
> наравне с плоской единым reader'ом (`.qwen/lib/frontmatter.sh::get_field`);
