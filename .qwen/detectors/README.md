...
3. Зарегистрировать в `config/capture-detectors.sh`:
  ```bash
  "NAME|.qwen/detectors/detector_NAME.sh|EVENT_TYPE|free|true|PostToolUse,Stop"
  ```
...
}' | .qwen/detectors/detector_incident.sh
```
...
Все срабатывания и ошибки — в `.qwen/logs/capture_log.jsonl`:

```bash
tail -f ~/IWE/.qwen/logs/capture_log.jsonl | jq .
```
