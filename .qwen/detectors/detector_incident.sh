...
#                              routing-карты DP.KR.001 §5 (кроме enumerated QWEN.md,
...
# (обычно знания уходят в docs/, inbox/, memory/ — корень для QWEN.md, README.md и т.п.)
matched=""
pattern=""
description=""
...
if [ "$TOOL_NAME" = "Write" ] && [[ "$FILE_PATH" == *.md ]]; then
  ...
  if [[ "$REL_PATH" != */* ]]; then
    ...
    case "$base" in
      QWEN.md|README.md|MEMORY.md|WORKPLAN.md|REGISTRY.md|MAP.md|STAGING.md|PROCESSES.md|CHANGELOG.md)
        : ;;
      *)
        matched="true"
