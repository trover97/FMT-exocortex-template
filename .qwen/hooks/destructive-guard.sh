...
if [ -n "$CWD" ]; then
  ...
  if [ "$CWD_PHYSICAL" != "$WORKSPACE_ROOT" ] && \
     echo "$CMD_EXEC" | grep -qE "(^|[[:space:]\"'])(\\.qwen/|scripts/|memory/)"; then
...
if [ -n "$RM_INVOCATIONS" ]; then
  while IFS= read -r one_rm; do
    ...
    if echo "$one_rm" | grep -qE '(/tmp/|/scratchpad/|\.qwen/worktrees/)'; then
      continue
    fi
    if echo "$CMD_EXEC" | grep -qE '(/tmp/|/scratchpad/|\.qwen/worktrees/)'; then
      continue
    fi
