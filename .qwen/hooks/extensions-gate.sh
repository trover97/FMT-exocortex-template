...
# Блокирует прямое редактирование .qwen/skills/ и memory/protocol-*.md
#
# Исключения:
...
if echo "$FILE_PATH" | grep -qE '\.qwen/skills/|memory/protocol-'; then
  ...
  echo '{"decision": "block", "reason": "⛔ Extensions Gate: платформенные (L1) и пользовательские (L3) файлы — разные слои. Правило (QWEN.md §9): Авторская кастомизация → extensions/*.md. Платформенное изменение → FMT-exocortex-template → update.sh. Смешение слоёв = хрупкость при обновлении. Создай или обнови нужный файл в extensions/."}'
  exit 0
fi
