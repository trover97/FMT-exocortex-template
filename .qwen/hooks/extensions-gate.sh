...
# произвольный Bash не разбирается (issue #528), что прямо раскрыто в QWEN.md.
# Блокирует прямое редактирование .qwen/skills/, memory/protocol-*.md и
...
#   - issue #311: новая директория .qwen/skills/<name>/, которой нет в
...
# Gate owns only the project workspace. A global ~/.qwen skill, a sibling
...
# глобальный ~/.qwen или соседний каталог с похожим именем (issue #528).
case "$REL_PATH" in
  .qwen/skills/*|memory/protocol-*)
  ...
  # Исключение 3 (issue #311): свой навык — .qwen/skills/<name>/ отсутствует
  # в манифесте платформы, update.sh его не затронет и не затрёт.
  # Требуем ИМЕННО поддиректорию (name/...) — плоский файл прямо в .qwen/skills/
  # (напр. SKILL-INDEX.yaml) НЕ подпадает и блокируется ниже.
  if printf '%s' "$REL_PATH" | grep -qE '^\.qwen/skills/[^/]+/'; then
    SKILL_NAME="${REL_PATH#\.qwen/skills/}"
    ...
    if [ -n "$SKILL_NAME" ]; then
      ...
      if jq -e '.files | type=="array" and length>0' "$MANIFEST" >/dev/null 2>&1; then
        IN_MANIFEST=$(jq -r --arg prefix ".qwen/skills/${SKILL_NAME}/" \
  ...
  block "платформенные (L1) и пользовательские (L3) файлы — разные слои. Правило (QWEN.md §9): Авторская кастомизация → extensions/*.md. Платформенное изменение → FMT-exocortex-template → update.sh. Смешение слоёв = хрупкость при обновлении. Создай или обнови нужный файл в extensions/."
  ;;
esac
