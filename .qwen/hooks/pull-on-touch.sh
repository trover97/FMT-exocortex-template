...
#
# Реализует правило Pull-on-Touch (QWEN.md §2 п.5) детерминированно, а не "по памяти агента".
...
#   Состояние: ~/.qwen/state/repo-pulled-<session>.txt (одно имя репо на строку).

set -uo pipefail

[[ "${1:-}" == "--help" ]] && {
    echo "pull-on-touch.sh — lazy 'git pull --rebase' on first repo touch per session (QWEN.md §2 п.5)"
    exit 0
}
