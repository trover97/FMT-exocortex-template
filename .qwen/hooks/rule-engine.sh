...
#
# REGISTRY: ~/IWE/.qwen/rules-registry.yaml (генерируется из PACK-agent-rules/)

set -uo pipefail
umask 077

REGISTRY="${RULE_REGISTRY:-$HOME/IWE/.qwen/rules-registry.yaml}"
...
SESSION_STATE_DIR="$HOME/.qwen/state"
...
check_arch_gate() {
    ...
    if echo "$file_path" | grep -qE '/PACK-[^/]+/(00-pack-manifest|pack-manifest)'; then
        emit_verdict "warn" "AR.003" "создание нового Pack — ArchGate обязателен ДО начала (QWEN.md §5). Запусти /archgate → ЭМОГССБ профиль"
        return
    fi
    ...
    if [[ "$event" == "new_system_creation" ]]; then
        ...
        if [ "$archgate_done" != "true" ]; then
            emit_verdict "warn" "AR.003" "создание новой системы без /archgate — запусти /archgate ДО реализации (QWEN.md §5, DP.ARCH.001 §7)"
            return
        fi
    fi
    ...
    if [[ "$event" == "arch_decision_attempt" ]]; then
        emit_verdict "warn" "AR.003" "архитектурное решение — /archgate ПЕРЕД финализацией (QWEN.md §5)"
        return
    fi
...
check_close() {
    ...
    if [ "$has_changes" = "true" ]; then
        emit_verdict "warn" "AR.005" "Close-триггер с незакоммиченными изменениями — запусти /run-protocol close (QWEN.md §2 п.3)"
        return
    fi
...
check_pull_on_touch() {
    ...
    if [ "$pull_done" = "true" ]; then
        ...
        emit_verdict "warn" "AR.006" "первая модификация в репо ${repo:-?} — выполни git pull --rebase ДО коммита (QWEN.md §2 п.4)"
    fi
}
...
check_checklist_verification() {
    ...
    if [ "$verification_done" = "true" ]; then
        ...
        emit_verdict "warn" "AR.007" "Close-попытка без R23 верификации — запусти sub-agent Haiku R23 (context isolation) ДО отчёта закрытия (QWEN.md §2 п.5)"
    fi
}
...
check_priority_gate() {
    ...
    if $needs_r; then
        if [ -z "$r_goal" ] || ! echo "$r_goal" | grep -qE 'R[0-9]+'; then
            emit_verdict "warn" "AR.012" "РП с budget=${budget}h (или open-loop) без явного R{N} — к какому результату недели ведёт? Добавь поле r_goal: R{N} во frontmatter (QWEN.md §2 Priority Gate)"
            return
        fi
    fi
...
_classify_integration_phase() {
    ...
    local classifier="$HOME/IWE/.qwen/scripts/integration-gate-classifier.py"
...
check_integration_gate() {
    ...
    if [ "${#missing[@]}" -gt 0 ]; then
        ...
        emit_verdict "warn" "AR.013" "IntegrationGate: создание нового ${creation_type:-инструмента/агента} без ${fallback_missing} — последовательность: обещание → сценарии → роль → реализация (QWEN.md §2 IntegrationGate). Пропуск = P10 (DP.FM.010)"
        return
    fi
...
check_legacy_port_gate() {
    ...
    emit_verdict "warn" "AR.014" "LegacyPortGate: замена ${legacy_component:-legacy-компонента} без субагент-исследования — запусти 15-мин субагент «как работает сейчас?» ДО проектирования (QWEN.md §2 LegacyPortGate). Пропуск = DP.FM.014"
}
...
check_routing_gate() {
    ...
    case "$target_path" in
        ...
        */.qwen/rules-registry.yaml|*/rules-registry.yaml) emit_verdict "ok" "AR.009" "known pattern (generated registry)"; return ;;
...
check_schema_registration_gate() {
    ...
    local cfg="${SCHEMA_TRIGGERS_CONFIG:-$HOME/IWE/.qwen/hooks/schema-triggers.yaml}"
    local in_scope
...
check_repo_touch_gate() {
    # AR.010: при первом касании репо — проверить QWEN.md на «обязательно загружай»
            ...
            if [[ "$rel_path" == "$ro" ]] || [[ "$rel_path" == "$ro/"* ]]; then
                case "$tool_name" in
                    Edit|Write|MultiEdit|NotebookEdit)
                        emit_verdict "block" "AR.010" "READ-ONLY репо ${ro} — ${tool_name} запрещён (QWEN.md §9). Только чтение."
                        return ;;
                esac
    ...
    local state_dir="$HOME/.qwen/state"
...
try:
    ...
    # Первое касание — отметить и проверить QWEN.md
...
if repo not in repos:
    ...
    local claudemd="$HOME/IWE/$repo/QWEN.md"
    if [ ! -f "$claudemd" ]; then
        emit_verdict "ok" "AR.010" "first touch repo $repo — no QWEN.md, gate N/A"
        return
    fi

    if grep -qE '(ОБЯЗАТЕЛЬНО ЗАГРУЖАЙ|обязательно загружай|load on touch)' "$claudemd" 2>/dev/null; then
        emit_verdict "warn" "AR.010" "первое касание репо $repo — QWEN.md содержит блок «ОБЯЗАТЕЛЬНО ЗАГРУЖАЙ», загрузить указанные файлы ДО ответа"
    else
        emit_verdict "ok" "AR.010" "first touch repo $repo — QWEN.md checked, no mandatory-load block"
    fi
}
...
check_release_verification_trigger() {
    ...
    if echo "$all_paths" | grep -qE 'update-manifest\.json'; then
        in_scope=true
    elif echo "$all_paths" | grep -qE '/(roles|\.qwen|setup|memory|extensions|seed)/'; then
        in_scope=true
    elif echo "$all_paths" | grep -qE 'FMT-exocortex-template/(roles|\.qwen|setup|memory|extensions|seed|update-manifest)'; then
        in_scope=true
    fi
    ...
    if [ "$verification_done" = "true" ]; then
        ...
        emit_verdict "warn" "AR.203" "изменение в FMT release scope ($(echo "$all_paths" | tr ' ' '\n' | grep -E 'update-manifest|/(roles|\.qwen|setup|memory|extensions|seed)/' | head -1 | xargs basename 2>/dev/null || echo 'scope file')) — запусти 5-слойную верификацию ДО push: pre-commit + CI + upgrade-test + detector regression + adversarial (VR.M.006, AR.203)"
    fi
}
...
state_dir = os.environ.get('_STATE_DIR', os.path.join(os.path.expanduser('~'), '.qwen', 'state'))
...
"
    ...
    test)
        ...
        run_test 39 "существующий registry-файл → ok (экземпляр/edit, не новая схема)" "ok" \
            RULE_EVENT="schema_registration_attempt" \
            RULE_CONTEXT="{\"target_path\":\"$HOME/IWE/.qwen/rules-registry.yaml\",\"is_new_file\":false}"
        ...
        if [ -z "$DOGFOOD_RESOLVED_PYTHON3" ]; then
            ...
            DOGFOOD=$(_REG="$REGISTRY" _CFG="${SCHEMA_TRIGGERS_CONFIG:-$HOME/IWE/.qwen/hooks/schema-triggers.yaml}" "$DOGFOOD_RESOLVED_PYTHON3" - <<'PYEOF' 2>/dev/null || echo "FAIL config unreadable"
import os, yaml
