...
GATE_LOG="$IWE_ROOT_GUESS/.qwen/logs/gate_log.jsonl"
...
if [ "$TOOL_NAME" = "Bash" ]; then
    ...
    NORM=$(printf '%s' "$CMD" | sed -E \
        ...
        -e "s@\\\$\\{IWE_TEMPLATE:-${WL_TEMPLATE_ABS//\//\\/}\\}/\\.qwen/scripts/memory-drift-scan\\.py@ __WL_PY_MDS__ @g" \
        -e "s@\\\$\\{IWE_TEMPLATE:-${WL_TEMPLATE_ABS//\//\\/}\\}/\\.qwen/scripts/check-index-health\\.py@ __WL_PY_CIH__ @g" \
    ...
    while IFS= read -r SEG; do
        ...
        case "$W0" in
            ...
            bash|sh|zsh)
                ...
                # (review-01 High, review-02 H1): glob */.qwen/... пропускал
                ...
                shift
                WL_ABS="$IWE_ROOT_GUESS/.qwen/scripts/load-extensions.sh"
                # Реальный deployed путь — вложенный клон, не workspace-root
                # (scripts/ не копируется в $WORKSPACE_DIR, в отличие от .qwen/;
                # см. IWE_SCRIPTS default в .qwen/lib/iwe-env-bootstrap.sh:86).
                ...
                case "${1:-}" in
                    .qwen/scripts/load-extensions.sh|"$WL_ABS") ;;
