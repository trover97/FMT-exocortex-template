...
# Лог: ~/.qwen/logs/style-violations.log
...
#
# Вход: Stop-хук Qwen Code передаёт JSON со stdin, поле transcript_path
...
COUNT_DIR="$PROJECT_DIR/.qwen/state"
...
LOG_FILE="${HOME}/.qwen/logs/style-violations.log"
...
echo "⚠️ Стиль: нарушения [${VIOLATIONS}]${ESC_NOTE} — подробности в ~/.qwen/logs/style-violations.log"
exit 0
