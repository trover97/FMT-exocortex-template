...
if [ "${IWE_SCOPE_HOOK_TRAMPOLINED:-0}" != "1" ] && [ -n "${QWEN_PROJECT_DIR:-}" ]; then
  PROJECT_TOP=$(git -C "$QWEN_PROJECT_DIR" rev-parse --show-toplevel 2>/dev/null || true)
  PRIMARY_TOP=$(_primary_worktree "$QWEN_PROJECT_DIR" 2>/dev/null || true)
  if [ -n "$PROJECT_TOP" ] && [ -n "$PRIMARY_TOP" ]; then
    PROJECT_TOP=$(cd "$PROJECT_TOP" 2>/dev/null && pwd -P) || exit 0
    if [ "$PROJECT_TOP" != "$PRIMARY_TOP" ]; then
      CANONICAL_HOOK="$PRIMARY_TOP/.qwen/hooks/post-tool-use-scope-track.sh"
      ...
      export QWEN_PROJECT_DIR="$PRIMARY_TOP"
