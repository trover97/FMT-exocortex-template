...
# install (scripts/lib/ lives inside the template or as .qwen/lib/, never
...
NO_RESOLVER_DIR="$TMP/no-resolver/.qwen/hooks"
...
WITH_RESOLVER_DIR="$TMP/with-resolver/.qwen/hooks"
mkdir -p "$WITH_RESOLVER_DIR" "$TMP/with-resolver/.qwen/lib"
cp "$HOOK_DIR/protocol-artifact-validate.sh" "$WITH_RESOLVER_DIR/"
cp "$REAL_RESOLVER" "$TMP/with-resolver/.qwen/lib/find-python3.sh"
