...
# Qwen Code itself knows "a function is starting": the Skill tool call. Before
...
# invoked through Qwen Code's own Skill tool. Point B (lazy, mid-execution
...
# code, not from a tool call Qwen Code can see. Point B stays a library call
# (residency-gate-lazy.sh) by design, not a gap this adapter forgot. The same
# is true for functions that never go through Qwen Code at all (day-open's
# own launchd-triggered pipeline, bot handlers) — residency-gate-init.sh
# remains their integration point; there is no Qwen Code hook to mechanize
# for a process Qwen Code never launches.
set -euo pipefail
...
MANIFEST="$PROJECT_ROOT/.qwen/skills/$SKILL_NAME/SKILL.md"
...
RESIDENCY_GATE_PY="$PROJECT_ROOT/.qwen/skills/residency-gate/residency-gate.py"
...
RUNNER="$PROJECT_ROOT/.qwen/lib/residency-gate-run.sh"
if [ ! -r "$RUNNER" ]; then
