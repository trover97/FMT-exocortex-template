...
#             надстройки в контекст Qwen Code ПЕРЕД ответом. Заменяет хрупкую
#             вклейку маркер-блока в генерируемые QWEN.md / AGENTS.md (которые
...
# jq нужен для вывода JSON. Нет jq → тихий валидный '{}', не пустой stdout (иначе Qwen Code не распарсит).
...
# JSON для Qwen Code UserPromptSubmit hook
