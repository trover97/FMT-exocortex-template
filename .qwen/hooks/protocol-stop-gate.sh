...
# Qwen Code запускает этот хук новым bash-процессом на каждое Stop-событие,
# поэтому env-переменная не переживает между вызовами — старый guard через
# STOP_HOOK_ACTIVE был мёртвым кодом с рождения. Qwen Code сам передаёт
...
GATE_LOG="$IWE_ROOT/.qwen/logs/gate_log.jsonl"
mkdir -p "$(dirname "$GATE_LOG")" 2>/dev/null || true

# --- Шаг 1: был ли вызов протокольного скилла? ---
# issue #758: в транскрипте Qwen Code tool_use лежит вложенно, в
