...
# Перехватывает PostToolUse и Stop события Qwen Code, эмитит agent.trace.* events
...
STATE_DIR="${HOME}/.qwen/state/agent-trace"
...
# Маппинг Qwen Code session_id → наш UUID v4 (для agent_trace.session.session_id PK).
