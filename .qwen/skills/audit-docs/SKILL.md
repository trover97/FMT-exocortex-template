...
1. Прочитать `<repo>/QWEN.md`; определить владельца и R5 Архитектора.
2. Показать им пример `.qwen/skills/audit-docs/.audit-context.yaml.example`.
...
1. `<repo>/QWEN.md` целиком — как любой агент в этом репо. В частности § 10 «Известные ловушки/инварианты» (если есть).
...
3. `${IWE_ROOT:-$HOME/IWE}/.qwen/sync-manifest.yaml` — найти пары, где `source` или `derived` пересекают этот репо. Использовать как дополнительный источник связей «код ↔ docs».
...
## Шаг 3. Связь с QWEN.md § 10

Для каждой ловушки/инварианта из § 10 QWEN.md репо проверить: упомянута ли в docs? Если нет — добавить в раздел «Неочевидности».
...
## Неочевидности (§ 10 QWEN.md, не покрыто docs)
- ...
