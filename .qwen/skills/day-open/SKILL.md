...
> **Шаблоны:** `.qwen/skills/day-open/templates.md` (читать перед шагами 7a и 7d).
...
### 0. Extensions (before)
`bash .qwen/scripts/load-extensions.sh day-open before` → Exit 0: Read каждый файл, выполнить. Exit 1: пропустить.
...
`content_editor.enabled: true` → топ-3 черновика из `drafts/` по актуальности/свежести/полноте → таблица в DayPlan. Сигнал готовых постов → «Требует внимания». Любое другое значение, включая отсутствие секции `content_editor` (issue #804) → пропустить.
...
### 6c. Extensions (after)
`bash .qwen/scripts/load-extensions.sh day-open after` → Exit 0: Read каждый файл, выполнить. Exit 1: пропустить.

### 7. Запись

> ⚠️ **Перед шагами 7a и 7d:** `Read .qwen/skills/day-open/templates.md`. Файл не найден → сообщить пилоту, не продолжать.
...
**7b.** `bash .qwen/scripts/load-extensions.sh day-open checks` → Exit 0: выполнить верификацию. БЛОКИРУЮЩЕЕ: commit запрещён до прохождения всех checks.
