...
- `--interrupt <id>` → Read `.qwen/skills/peer-conversation/modes.md`. Перейти к **Шагу 5 (interrupt-режим)**. Стоп после.
- `--finalize <id>` → Read `.qwen/skills/peer-conversation/modes.md`. Перейти к **Шагу 6 (finalize-режим)**. Стоп после.
...
Найти WP по задаче: прочитать `{{GOVERNANCE_REPO}}/WP-REGISTRY.md` (grep по ключевым словам) и `{{GOVERNANCE_REPO}}/current/WeekPlan W{N}.md`.
...
```
# informational — pilot selects model at Qwen Code startup, not auto-applied here
...
(Класс задачи: <verification_class>. Выбери модель при запуске Qwen Code.)
```
...
> Statefulness-пробел Kimi закрыт автопередачей git-diff в `kimi-peer-adapter.sh` (§8).
...
```bash
SESSIONS_DIR="$HOME/IWE/{{GOVERNANCE_REPO}}/sessions"
TODAY=$(date +%Y-%m-%d)
MONTH=$(date +%Y-%m)
...
echo "<промпт>" | bash "$HOME/IWE/{{GOVERNANCE_REPO}}/scripts/kimi-peer-adapter.sh" \
  --add-dir "$SESSION_DIR" \
...
Записать `{{GOVERNANCE_REPO}}/sessions/<MONTH>/<TODAY>-<SESSION_SLUG>.md` (Write):
```markdown
---
date: <TODAY>
type: peer-session
...
> Режимы `--interrupt`, `--finalize` и верификация → `.qwen/skills/peer-conversation/modes.md`
