...
`bash .qwen/scripts/load-extensions.sh strategy-session before` → Exit 0: Read каждый файл, выполнить. Exit 1: пропустить.
...
```bash
SESSIONS_DIR=$(source $IWE_SCRIPTS/lib/common.sh 2>/dev/null && iwe_sessions_dir 2>/dev/null) || SESSIONS_DIR="{{WORKSPACE_DIR}}/{{GOVERNANCE_REPO}}/sessions"
...
В `~/.qwen/projects/{{CLAUDE_PROJECT_SLUG}}/memory/MEMORY.md` добавь раздел «РП текущей недели» со списком из 2.3.
...
**Extensions (after):** `bash .qwen/scripts/load-extensions.sh strategy-session after` → Exit 0: Read каждый файл, выполнить. Exit 1: пропустить.
