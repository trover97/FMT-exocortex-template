...
> **Scope:** create a new skill OR guide an update to an existing skill in `.qwen/skills/` or `.kimi/skills/`.
...
2. **Routing Gate.** Decide target platform (Claude vs Kimi) and level (project vs user). Default: project-level `.qwen/skills/` in the author repo.
...
Target path:
- Claude project: `.qwen/skills/<name>/`
- Kimi project: `.kimi/skills/<name>/`
- User level: `~/.qwen/skills/<name>/` or `~/.kimi/skills/<name>/`
