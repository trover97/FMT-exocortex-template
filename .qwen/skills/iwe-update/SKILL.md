...
```bash
bash .qwen/scripts/load-extensions.sh iwe-update before
```
...
- Нет → предупредить: «Первое обновление после миграции. QWEN.md будет обновлён с сохранением USER-SPACE секции. Рекомендую проверить результат.»
...
Если QWEN.md содержит маркеры `<<<<<<<` → помочь разрешить:
...
```bash
bash .qwen/scripts/load-extensions.sh iwe-update checks
```
...
Перезапустите Qwen Code для применения обновлений memory/.
```

### 7. Extensions `after`

```bash
bash .qwen/scripts/load-extensions.sh iwe-update after
```
