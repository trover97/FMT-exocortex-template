...
**Выход:** восстановленные `memory/` + `extensions/` + `QWEN.md` + симлинк `memory/`; отчёт + напоминание перезапустить Qwen Code.
...
- Определить целевую auto-memory: `~/.qwen/projects/$(echo "$HOME" | tr '/_.' '-')-IWE/memory` (Qwen Code слугифицирует `/`, `_`, `.` → `-`; `tr '/' '-'` промахнётся при `_` в username).
...
### Шаг 4. Отчёт
- Сколько memory-файлов восстановлено, восстановлен ли `QWEN.md`, создан ли симлинк.
- **Напомнить: перезапустить Qwen Code** — память грузится при старте сессии.
