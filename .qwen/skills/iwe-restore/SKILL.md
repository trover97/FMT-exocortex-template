---
name: iwe-restore
description: "Восстановление памяти агента из exocortex-бэкапа при переезде на новое устройство. Находит DS-strategy/exocortex/, показывает что будет восстановлено, спрашивает подтверждение, копирует memory/ + QWEN.md + AGENTS.md, финализирует /audit-installation."
...
Восстановление памяти агента из exocortex-бэкапа при переезде на новое устройство. Находит DS-strategy/exocortex/, показывает что будет восстановлено, спрашивает подтверждение, копирует memory/ + QWEN.md + AGENTS.md, финализирует /audit-installation.
...
MEMORY_DST="$HOME/.qwen/projects/${HOME_SLUG}-IWE/memory"
```
...
Найден бэкап:
  ...
  Назначение: ~/.qwen/projects/.../memory/
...
```bash
rsync -av --exclude='QWEN.md' --exclude='AGENTS.md' \
...
if [ -f "$EXOCORTEX/QWEN.md" ]; then
  # Если QWEN.md уже есть — показать: «QWEN.md уже существует. Перезаписать?»
  # При согласии:
  sed "s|{{HOME_DIR}}|$HOME_SED_SAFE|g" "$EXOCORTEX/QWEN.md" > "$WORKSPACE_DIR/QWEN.md"
fi
```
...
> «Память восстановлена. Перезапусти Qwen Code — агент подхватит файлы из `memory/` при следующей сессии.»
...
| `QWEN.md` в workspace новее бэкапа | Предупредить: «Файл в workspace новее бэкапа на N дней. Перезаписать потеряешь правки.» |
