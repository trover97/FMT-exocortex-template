...
### 6.7. Раздача скиллов в `.qwen/skills/` пилотского репо

**Зачем:** скиллы (`/lesson`, `/lesson-close`, `/connect-guide`, `/personal-guide-render`, `/personal-guide-start`) живут в `~/IWE/.qwen/skills/` платформы. При работе пилота в **claude.ai/code** (cloud sandbox) user-global `~/.qwen/skills/` не пробрасывается. Чтобы скиллы работали в любом канале (VS Code, CLI, browser), нужно их положить в **сам репо** пилота.

Идемпотентная проверка:
1. Через `personal_search(source: "personal-guide", path: ".qwen/skills/lesson/SKILL.md")` проверь, есть ли уже скилл в репо.
...
**Failure mode:** если `~/IWE/.qwen/skills/` недоступен (платформа другая) — пропустить шаг с warning. Не блокер для остальных 6 файлов.
