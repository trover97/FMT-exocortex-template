...
>
> Различение зафиксировано в `.qwen/rules/distinctions.md` (AUTHOR-ONLY).
...
> **Зачем:** скиллы (`/lesson`, `/lesson-close`, `/connect-guide`, `/personal-guide-render`, `/personal-guide-start`) живут в `~/IWE/.qwen/skills/` платформы. При работе пилота в **claude.ai/code** (cloud sandbox) user-global `~/.qwen/skills/` не пробрасывается. Чтобы скиллы работали в любом канале, их нужно положить в сам репо пилота.
...
for skill_path in SKILLS_TO_DISTRIBUTE:
    content = Read(f"~/IWE/.qwen/skills/{skill_path}")
    personal_write(
        source="DS-personal-guide",
        path=f".qwen/skills/{skill_path}",
        content=content,
    )
```
...
### QWEN.md — доменная кастомизация (WP-527 Ф4)

> **Зачем не просто «если нет».** С WP-527 Ф4 Шаг 1 сам проходит через `/repo-new` Execute (Step 8), который уже разворачивает generic `QWEN.md`-заглушку как часть скелета — при первом создании репо там будет именно она, не пусто. Этот шаг заменяет её на доменную версию (render-скилл, конкретный процесс работы). Но **не перезаписывать безусловно** — Контракт скилла обещает идемпотентность повторного вызова, а `QWEN.md` явно называет пилота писателем (`Writer: пилот`); безусловная перезапись на втором вызове стирала бы правки пилота, которые он имеет право туда вносить.

Через `personal_search(source: "DS-personal-guide", path: "QWEN.md")` прочитать текущее содержимое. Записать (перезаписать) доменную версию **только если** файл отсутствует ИЛИ его текст содержит маркер generic-заглушки `/repo-new` — «Это заглушка, развёрнутая скиллом `/repo-new`» (пункт 3 `assets/repository-skeleton/QWEN.md`) — то есть репозиторий только что создан Execute и ещё не кастомизирован. Если файл существует и маркера нет — считать его уже кастомизированным (этим же шагом ранее или пилотом вручную) и не трогать:

```python
personal_write(
    source="DS-personal-guide",
    path="QWEN.md",
    content="""# DS-personal-guide — инструкции для Qwen Code
...
```python
template_content = Read("~/IWE/.qwen/skills/personal-guide-render/templates/reflection-template.md")
personal_write(
    source="DS-personal-guide",
...
Если Skill tool недоступен (например, отладка вне Qwen Code) — fallback: прочитай `~/.qwen/skills/personal-guide-render/SKILL.md` и выполни Шаги 1-7 инлайн.
...
2. **Скиллы установлены.** `personal_search(source: "DS-personal-guide", path: ".qwen/skills/personal-guide-start/SKILL.md")` — нашёл. Не нашёл → Шаг 1.5 не завершился, перезапусти Шаг 1.5.
