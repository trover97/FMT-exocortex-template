---
name: setup-wakatime
description: Настройка WakaTime time-tracking для Qwen Code и VS Code
user_invocable: true
routing:
  executor: script
  deterministic: true
  script_path: ".qwen/skills/setup-wakatime/setup.sh"
---
...
2. **Хук Qwen Code** — автоматический трекинг при работе с Claude (категория "AI Coding")
...
```bash
mkdir -p ~/.qwen/hooks
cp .qwen/hooks/wakatime-heartbeat.sh ~/.qwen/hooks/wakatime-heartbeat.sh
chmod +x ~/.qwen/hooks/wakatime-heartbeat.sh
```

### Шаг 4: Настройка хуков в settings.json

Прочитай `~/.qwen/settings.json`. Добавь в секцию `hooks` (не затирая существующие хуки):

- **UserPromptSubmit** — добавь hook group:
  ```json
  {"hooks": [{"type": "command", "command": "~/.qwen/hooks/wakatime-heartbeat.sh"}]}
  ```
- **PostToolUse** — добавь:
  ```json
  {"hooks": [{"type": "command", "command": "~/.qwen/hooks/wakatime-heartbeat.sh", "async": true}]}
  ```
- **Stop** — добавь:
  ```json
  {"hooks": [{"type": "command", "command": "~/.qwen/hooks/wakatime-heartbeat.sh", "async": true}]}
...
```bash
echo '{"cwd": "'$(pwd)'", "hook_event_name": "UserPromptSubmit", "prompt": "test"}' | ~/.qwen/hooks/wakatime-heartbeat.sh
sleep 3
~/.wakatime/wakatime-cli --today
```

Покажи результат пользователю. Скажи: «Хуки подхватятся при следующем запуске Qwen Code (хуки загружаются при старте сессии).»
