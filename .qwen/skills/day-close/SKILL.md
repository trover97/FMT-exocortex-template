...
### 0. Extensions (before)
Загрузить: `bash .qwen/scripts/load-extensions.sh day-close before`. Exit 0 → `Read` каждый файл из вывода (alphabetic) → выполнить как первые шаги. Exit 1 → пропустить. Поддерживает `extensions/day-close.before.md` И `extensions/day-close.before.<suffix>.md`.

### 1. Extensions (checks)
Загрузить: `bash .qwen/scripts/load-extensions.sh day-close checks`. Exit 0 → `Read` каждый файл из вывода (alphabetic) → выполнить. Exit 1 → пропустить. Поддерживает `extensions/day-close.checks.md` И `extensions/day-close.checks.<suffix>.md`.

### 1b. Extensions (after)
Загрузить: `bash .qwen/scripts/load-extensions.sh day-close after`. Exit 0 → `Read` каждый файл из вывода (alphabetic) → выполнить (smoke-тесты, week-draft-append, доп. проверки). Exit 1 → пропустить. Поддерживает `extensions/day-close.after.md` И `extensions/day-close.after.<suffix>.md`.
...
for repo in $(ls {{HOME_DIR}}/IWE/); do
  if [ -d {{HOME_DIR}}/IWE/$repo/.git ]; then
    commits=$(git -C {{HOME_DIR}}/IWE/$repo log --since="today 00:00" --oneline --no-merges 2>/dev/null \
      ...
      | grep -vE "memory/|\.qwen/rules/|template-sync|backup|reindex" \
      || true)
...
**3c.** Обновить `${IWE_GOVERNANCE_REPO:-DS-strategy}/docs/WP-REGISTRY.md`: статусы + даты + **done-форматирование**. Done-РП → зачеркнуть номер, приоритет, название, репо, бюджет (`~~...~~`); снять bold с названия; эмодзи ✅ НЕ зачёркивать (см. `.qwen/rules/formatting.md §Таблицы с РП`). Тильду внутри ячеек заменить (`~6.5h` → `6.5h`).
...
grep -nE "→ ждёт|ждёт|dep:|блокер|blocked:|остановлен|ждёт согласования" \
  {{HOME_DIR}}/.qwen/projects/*/memory/MEMORY.md 2>/dev/null
```
...
```bash
SCRIPT="{{HOME_DIR}}/IWE/.qwen/scripts/check-index-health.py"
...
Скрипт выполняет: Linear sync, downstream sync (update.sh), backup (memory/ + QWEN.md).
...
```bash
SCRIPT="$HOME/IWE/.qwen/scripts/rule-classifier.py"
...
- [ ] Repo QWEN.md: feat-коммиты → новые правила?
...
- [ ] **Rule-engine FP-stats** (WP-272 Ф2.5): `[ -f ~/IWE/.qwen/scripts/fp-stats.py ] && python3 ~/IWE/.qwen/scripts/fp-stats.py --date $(date +%Y-%m-%d) || echo "skip: fp-stats.py требует rule-classifier.py"` → если есть `⚠️ REVISE` (FP > 20%) — записать в «Завтра начать с» правило + FP%. Флоу ревизии: `~/IWE/PACK-agent-rules/revision-flow.md`.
