...
- **Фаза B — исполнитель.** ОДИН general-purpose субагент (sonnet). В промпт: (а) дайджест целиком, (б) сегодняшняя дата, (в) инструкция: «Прочитай `.qwen/skills/day-close/SKILL.md`, выполни шаги 1-7 через TodoWrite, применяя карту замен ниже. Шаг 0 пропусти — родитель уже выполнил. Форматирование таблиц — `.qwen/rules/formatting.md`. Git: стейджить только конкретные файлы, НЕ коммитить. Верни: черновик итогов (шаг 7), список правленных файлов, блокеры». Приложить карту замен.
...
PY3="$(bash "$T/.qwen/lib/find-python3.sh")"
...
grep -nE "→ ждёт|ждёт|dep:|блокер|blocked:|остановлен|ждёт согласования" \
  $HOME/.qwen/projects/*/memory/MEMORY.md 2>/dev/null
```
...
```bash
T="${IWE_TEMPLATE:-$HOME/IWE/FMT-exocortex-template}"
PY3="$(bash "$T/.qwen/lib/find-python3.sh")" && "$PY3" "$T/.qwen/scripts/check-index-health.py"
```
