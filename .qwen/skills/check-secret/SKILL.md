...
routing:
  executor: script
  deterministic: true
  script_path: ".qwen/skills/check-secret/check.sh"
...
>
> **Архитектурное ограничение** (см. B7.7 в WP-212): не покрывает Claude-generated text без tool-use — для этого нужен внешний wrapper над Qwen Code.
...
Каждое использование скилла логируется в `~/IWE/.qwen/logs/check-secret.jsonl` (только metadata: timestamp, hash аргумента, decision; **не сами секреты**).
