...
```bash
bash .qwen/scripts/load-extensions.sh archgate before
```
...
**Черновик DRR:** подготовь данные по шаблону `.qwen/templates/drr-template.md`,
...
```bash
bash .qwen/scripts/load-extensions.sh archgate checks
```
...
по шаблону `.qwen/templates/drr-template.md`. При `core_result = rejected` не
...
```bash
bash .qwen/scripts/load-extensions.sh archgate after
```
