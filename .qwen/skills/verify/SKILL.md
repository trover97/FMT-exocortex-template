...
```bash
bash .qwen/scripts/load-extensions.sh verify before
```
...
- Прочитать `QWEN.md` затронутого репо
- Передать sub-agent'у: diff + QWEN.md + чеклист code
...
- **Этап 🟡 (два специализированных sub-agent, Opus, context isolation) — WP-322 Ф14:**
  ...
  **Sub-agent 1 — verify-fpf (промпт: `verify-fpf-subsection.md`):**
  ...
  - Промпт: `.qwen/skills/verify/verify-fpf-subsection.md`
  - Модель: Opus
  ...
  **Sub-agent 2 — verify-pedagogy (промпт: `verify-pedagogy-subsection.md`):**
  - Вход: файл подраздела
  - Промпт: `.qwen/skills/verify/verify-pedagogy-subsection.md`
  - Модель: Opus
...
| Код (DS) | QWEN.md репо + Pack-описания сервисов |
...
```bash
bash .qwen/scripts/load-extensions.sh verify checks
```
...
```bash
bash .qwen/scripts/load-extensions.sh verify after
```
