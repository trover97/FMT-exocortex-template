...
LOG_FILE="${CAPTURE_LOG_FILE:-$HOME/IWE/.qwen/logs/capture_log.jsonl}"
...
CAPTURE_CONFIG="${CAPTURE_CONFIG:-${IWE_ROOT:-$HOME/IWE}/.qwen/capture-config.sh}"
...
case "$EVENT_TYPE" in
  ...
  decision_user)
    ...
    ;;
  gate_fired)
    REL_PATH=".qwen/logs/gate-log-${YYYY_MM}.jsonl"
    FORMAT="jsonl"
    ;;
  archgate_result)
