...
if [ -z "${WORKSPACE_DIR:-}" ]; then
  ...
  if [[ "$_IWE_BOOTSTRAP_DIR" =~ /FMT-exocortex-template/scripts ]]; then
  ...
  elif [[ "$_IWE_BOOTSTRAP_DIR" =~ /\.qwen/ ]]; then
    # We're in .qwen/hooks, .qwen/lib, .qwen/detectors, or .qwen/skills
    # When bootstrap is sourced from inside FMT-exocortex-template/.qwen/,
    ...
    echo "    - $WORKSPACE_DIR/.qwen/{lib,detectors,hooks,skills}/..." >&2
...
export IWE_ROOT
# IWE_WORKSPACE: alias for WORKSPACE_DIR, referenced by SPF/Pack QWEN.md paths.
